...
def assignment_loop(network: FlowTransportNetwork,
    ...
    while gap > accuracy:
        ...
        iteration_values.append(iteration_number)
        ...
        if iteration_number > maxIter:
            if verbose:
                ...
                data1 = {'Iteration': iteration_values, 
                        'Error': gap_values} 
                df1 = pd.DataFrame(data1)
                excel_file_path = 'iteration_values.xlsx' 
...
def writeResults(network: FlowTransportNetwork, output_file: str, costFunction=BPRcostFunction,
    ...
    tmpOut = "init_node\tterm_node\tfreetime\tcapacity\tflow\tV/C\ttravelTime"
    outFile.write(tmpOut + "\n")
    ...
    for i in network.linkSet:
        color = ("limegreen" if (network.linkSet[i].flow / network.linkSet[i].capacity) <= 0.5
                 else "darkgreen" if 0.5 < (network.linkSet[i].flow / network.linkSet[i].capacity) < 0.77
                 else "salmon" if 0.77 <= (network.linkSet[i].flow / network.linkSet[i].capacity) < 1           
                 else "red")
        link_row = [network.linkSet[i].init_node, 
                    network.linkSet[i].term_node,
                    network.linkSet[i].fft,
                    network.linkSet[i].capacity,
                    network.linkSet[i].flow,
                    network.linkSet[i].flow / network.linkSet[i].capacity,
                    (costFunction(False,
                                  network.linkSet[i].fft,
        ...
        results.append(link_row)
    
    df2 = pd.DataFrame(results, columns=['init_node', 'term_node', 'freetime', 'capacity', 'flow', 'V/C', 'travelTime', 'Color'])
    excel_file_path2 = 'Results.xlsx'
    ...
    for i in network.linkSet:
        tmpOut = str(network.linkSet[i].init_node) + "\t" + str(
            ...
            network.linkSet[i].fft)+ "\t" + str(
            network.linkSet[i].capacity) + "\t" + str(
            network.linkSet[i].flow)  + "\t" + str(
            network.linkSet[i].flow / network.linkSet[i].capacity) + "\t" + str(costFunction(False,
                                                               network.linkSet[i].fft,
                                                               network.linkSet[i].alpha,
                                                               network.linkSet[i].flow,
                                                               network.linkSet[i].max_capacity,
